...
def create_synthesized_image(image_width, image_height, num_dots_range, dot_size_range, noise_scale):
    ...
    # Generate a random number of dots within a specified range
    num_dots = random.randint(*num_dots_range)

    # List to store labels for each dot
    labels = []

    # Draw random dots on the image and calculate labels
    for _ in range(num_dots):
        ...
        # Calculate label values normalized to image dimensions
        ...
        # Append label string to the list
        labels.append(f"0 {center_x} {center_y} {width} {height}")

    # Convert the image to array and add noise
...
# Create and save 250 synthetic images with labels
for i in range(250):
